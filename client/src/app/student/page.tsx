...
import {searchResult} from "@/lib/results";
...
function InputWithButton({onSearch}) {
	...
	return (
		<div className="flex w-full max-w-sm items-center space-x-2">
			<Input
				type="text"
				placeholder="NAME or RANK or CET-ROLL-# (AB123)"
...
export default function Page() {
	...
	useEffect(() => {
		async function fetchDetails() {
			...
			let type: "reg" | "rank" | "name";
			if (!/\d/.test(searchQuery)) type = "name";
			else {
				if (searchQuery.charAt(0).match(/[a-zA-Z]/)) type = "reg";
				else type = "rank";
			}

			const results: ResultData[] = await searchResult(searchQuery, type);
			let rawResults = [];
			DataMap.clear();
